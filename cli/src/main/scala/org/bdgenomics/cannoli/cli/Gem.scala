...
import org.bdgenomics.cannoli.{ Gem => GemFn, GemArgs => GemFnArgs }
...
/**
 * GEM-Mapper command line arguments.
 */
class GemArgs extends GemFnArgs with ADAMSaveAnyArgs with CramArgs with ParquetArgs {
...
/**
 * GEM-Mapper command line wrapper.
 */
class Gem(protected val args: GemArgs) extends BDGSparkCommand[GemArgs] with Logging {
  val companion = Gem
  ...
  def run(sc: SparkContext) {
    ...
    val alignments = new GemFn(args, sc).apply(fragments)
