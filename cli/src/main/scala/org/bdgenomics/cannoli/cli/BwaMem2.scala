...
import org.bdgenomics.cannoli.{ BwaMem2 => BwaMem2Fn, BwaMem2Args => BwaMem2FnArgs }
import org.bdgenomics.cannoli.util.QuerynameGrouper
import org.bdgenomics.utils.cli._
import org.kohsuke.args4j.{ Argument, Option => Args4jOption }

object BwaMem2 extends BDGCommandCompanion {
  val commandName = "bwaMem2"
  val commandDescription = "Align paired-end reads in a fragment dataset with Bwa-mem2."

  def apply(cmdLine: Array[String]) = {
    new BwaMem2(Args4j[BwaMem2Args](cmdLine))
  }
}
...
/**
 * Bwa-mem2 command line wrapper.
 */
class BwaMem2(protected val args: BwaMem2Args) extends BDGSparkCommand[BwaMem2Args] with Logging {
  val companion = BwaMem2
  ...
  def run(sc: SparkContext) {
    ...
    val output: AlignmentDataset = new BwaMem2Fn(args, sc).apply(input)
