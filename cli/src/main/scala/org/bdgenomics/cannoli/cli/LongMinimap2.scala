...
import org.bdgenomics.cannoli.{ LongMinimap2 => LongMinimap2Fn, Minimap2Args => Minimap2FnArgs }
import org.bdgenomics.utils.cli._
import org.kohsuke.args4j.{ Argument, Option => Args4jOption }

object LongMinimap2 extends BDGCommandCompanion {
  val commandName = "longMinimap2"
  val commandDescription = "Align long reads in a sequence dataset with Minimap2."

  def apply(cmdLine: Array[String]) = {
    new LongMinimap2(Args4j[LongMinimap2Args](cmdLine))
  }
}

/**
 * Long read Minimap2 command line arguments.
 */
class LongMinimap2Args extends Minimap2FnArgs with ADAMSaveAnyArgs with ParquetArgs {
...
/**
 * Long read Minimap2 command line wrapper.
 */
class LongMinimap2(protected val args: LongMinimap2Args) extends BDGSparkCommand[LongMinimap2Args] with Logging {
  val companion = LongMinimap2

  def run(sc: SparkContext) {
    val sequences = sc.loadDnaSequences(args.inputPath)
    val alignments = new LongMinimap2Fn(args, sc).apply(sequences)
