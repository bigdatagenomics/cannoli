...
class GemArgs extends Args4jBase {
  ...
  @Args4jOption(required = false, name = "-image", usage = "Container image to use. Defaults to quay.io/biocontainers/gem3-mapper:3.6.1--h67092d7_10.")
  var image: String = "quay.io/biocontainers/gem3-mapper:3.6.1--h67092d7_10"
...
/**
 * GEM-Mapper wrapper as a function FragmentDataset &rarr; AlignmentDataset,
...
class Gem(
  ...
  override def apply(fragments: FragmentDataset): AlignmentDataset = {
    ...
    info("Piping %s to GEM-Mapper with command: %s files: %s".format(
