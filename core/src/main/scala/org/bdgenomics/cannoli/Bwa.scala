...
import org.bdgenomics.adam.models.{ ReadGroup => ReadGroupModel, ReadGroupDictionary }
...
import org.bdgenomics.formats.avro.{ Alignment, ReadGroup }
...
class BwaArgs extends ReadGroupArgs {
  ...
  @Args4jOption(required = false, name = "-image", usage = "Container image to use. Defaults to quay.io/biocontainers/bwa:0.7.17--hed695b0_6.")
  var image: String = "quay.io/biocontainers/bwa:0.7.17--hed695b0_6"
...
class Bwa(
  ...
  override def apply(fragments: FragmentDataset): AlignmentDataset = {
    ...
    info("Piping %s to bwa with command: %s files: %s".format(
